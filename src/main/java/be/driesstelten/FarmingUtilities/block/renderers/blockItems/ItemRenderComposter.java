package be.driesstelten.FarmingUtilities.block.renderers.blockItems;
...
public class ItemRenderComposter implements IItemRenderer {
	
	private ModelComposter model;
	
	public ItemRenderComposter(ModelComposter model) {
		this.model = model;
	}
...
//			//break;
	...
	protected void bindTexture(Block block, int meta) {
        ...
        if (texturemanager != null)
        {
        	if (meta >= 0)
    		{
    			texturemanager.bindTexture(model.getComposterTexture(block, meta));
    		}
        }
    }
